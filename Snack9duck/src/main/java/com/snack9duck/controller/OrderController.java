   ...
   @RequestMapping(value="order_submit", method = { RequestMethod.GET, RequestMethod.POST })
      public String OrderSubmitPost(HttpServletRequest request, Model model, OrderVO order){
        HttpSession session = request.getSession();
         ...
         List<OrderVO> orderList = service.listOrderById(orderVO);
         log.info("orderList안에는"+ orderList);
         service.updateOrderList(orderList);
         OrderVO order2 = new OrderVO();
         order2.setRequest(order.getRequest());
         order2.setOseq(orderVO.getOseq());
         log.info("-----------------------request :"+order.getRequest());
         log.info("-----------------------order :"+order);
         service.updateRequest(order2);
