...
//	}
	...
	@RequestMapping(value = "order/admin_order_Update", method = RequestMethod.GET)
	public void orderDetailGET(Model model, @RequestParam(value="odseq", required=false)int odseq) throws Exception {
//		model.addAttribute("orderList", orderService.adminOrderList());
		model.addAttribute("orderList",orderService.adminOrderModify(odseq));
		logger.info("관리자 구독 관리 페이지 진입");
	}

	
	@PostMapping("order/updateOrderResult3")
	public String updateOrderResult3(OrderVO order) {
			
			orderService.updateOrderResult3(order);
			log.info(order);
			return "redirect:/admin/order/admin_order_main";
		}
