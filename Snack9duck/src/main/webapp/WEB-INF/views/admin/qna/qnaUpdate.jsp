...
<c:choose>
	...
	<article>
		...
		<form name="formm" method="post" action="/admin/qna/qnaUpdate">
		<input type="hidden" name="qseq" value="${qna.qseq}">
			<table id="notice">
				...
				<tr style="border: none;">
					<th>답변 내용</th>
						...
						<textarea name="reply" cols="70"rows="10" style="font-size: 17px; margin-top:10px;">
							${qna.reply}
						</textarea>
				</tr>
			</table>
			...
			<div id="buttons" style="float: right; margin: 20px 0">
				...
				<input type="button" value="답변하기" class="submit" onclick="go_register()" style="padding: 3px 10px; margin-top:10px">
					</c:when>
					<c:otherwise>
					<input type="button" value="답변 수정하기" class="submit" onclick="go_update()" style="padding: 3px 10px; margin-top:10px">
					</c:otherwise>
					</c:choose>
			</div>
		</form>
...
function go_register(){
	alert("답변이 등록 되었습니다.");
	formm.action="/admin/qna/qnaUpdate";
	formm.submit();
}
function go_update(){
	alert("답변이 수정 되었습니다.");
